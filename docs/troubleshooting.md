...
`DEEPPAIRING_PORT_BASE` values are preserved when the resulting window is valid;
invalid or overflowing bases use an isolated derived window. Invalid
`DEEPPAIRING_PORT_SPAN` values normalize to 128. Setting a valid base explicitly
may intentionally select the normal application range.
