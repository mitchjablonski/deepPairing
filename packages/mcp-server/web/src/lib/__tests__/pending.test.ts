...
describe("computePending — single source of truth for 'waiting on human'", () => {
  it("counts draft reviewable artifacts (research/spec/plan/decision/code_change/changeset) but not reasoning", () => {
    const artifacts = [
    ...
    expect(drafts.map((a) => a.id).sort()).toEqual(["cc", "cs", "d", "p", "r", "spec"]);
    expect(total).toBe(6);
  });
