    ...
    tools: [
      {
        ...
        description:
          `Present research findings as a structured artifact in the companion UI (${port ? `localhost:${port}` : ""}). Each finding carries evidence, category, significance, severity, and a \`concept\` naming the pattern.` +
          `\n\nSchema note: \`findings\` is an array of objects (NOT a string). Required per-finding: category, detail, significance. Name each finding's \`concept\` ({name, oneLineExplanation?}) — the PREFERRED place to name a pattern (ahead of log_reasoning); it renders as a ledger-aware badge. INPUT_VALIDATION_FAILED on mismatch, with the bad path + an example.` +
      ...
      },
      {
        ...
        description:
          ...
          "\n\nSchema note: `title` (artifact-level) and `files` are REQUIRED. Each file has `path`, `changeType` ('modified'|'added'|'deleted'), and `hunks` (unified-diff shaped: an optional `header` plus `lines`, each `{ kind: 'ctx'|'add'|'del', content, oldLine?, newLine? }`). Optional `summary`, `risks[]` (e.g. 'touches auth'), and per-file `stats` ({additions, deletions}). Optional `visuals[]` — DRAW THE BLAST RADIUS: a diagram or file_map of the shape of what this change touches, rendered above the file rail so the human sees the scope before diving into hunks. INPUT_VALIDATION_FAILED on mismatch." +
      ...
      },
      {
        ...
        description:
          ...
          "- `any` (default) — unions philosophy + sessions, philosophy first. Requires a query (a bare `recall(mode='any')` errors — pass the concept you're about to propose, or use `mode='philosophy'` with an empty query to list the ledger).",
        inputSchema: {
          type: "object" as const,
          properties: {
