...
type ExportFormat = "full" | "pr-description" | "adr" | "replay" | "pr-review";
...
export function buildGitHubReviewPayload(
  ...
  const bodyParts: string[] = [
    `## deepPairing review — ${title}`,
    "",
  ];
  if (comments.length === 0) {
    bodyParts.push("_No reviewable findings with structured evidence in this session._");
  } else {
    bodyParts.push(`${comments.length} inline comment${comments.length === 1 ? "" : "s"} on findings from this deepPairing session.`);
...
// --- PR-review format (deepPairing findings as GitHub-style PR review comments) ---
//
// Produces markdown ready to paste into a GitHub PR review comment box.
// Each approved finding with structured evidence becomes a quoted file:line
// reference + severity chip + impact/recommendation. Findings with a
// `suggestion` on their evidence become GitHub ```suggestion blocks so the
// reviewer can one-click-apply. Rejected findings are omitted (they weren't
// worth the review comment).

function formatPrReview(state: SessionState): string {
  const sections: string[] = [];
  const title = getSessionTitle(state);
  sections.push(`## deepPairing review — ${title}`);
  sections.push("");
  ...
  if (allFindings.length === 0) {
    sections.push("_No reviewable findings in this session._");
    sections.push("");
    return sections.join("\n");
  }
  ...
  sections.push("*Generated by [deepPairing](https://github.com/deeppairing) — paste this into a GitHub PR review comment.*");
  return sections.join("\n");
}
